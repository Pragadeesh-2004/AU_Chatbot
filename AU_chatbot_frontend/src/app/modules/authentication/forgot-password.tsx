...
// ✅ College options - same as login page
...
export default function ForgotPasswordPage({
    ...
    const [college, setCollege] = useState("Anna_university"); // ✅ Add college state
    ...
    const handleSendCode = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/authentication/forgot-password/send-code`, {
                ...
                body: JSON.stringify({ college, role, id }), // ✅ Include college
            });
    ...
    const handleResetPassword = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/authentication/forgot-password/reset`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ college, role, id, code, password }), // ✅ Include college
            });
            const data = await res.json();
            if (!res.ok) {
                let errorMessage = "Something went wrong. Please try again.";
        ...
        <div className="min-h-[400px] flex items-center justify-center">
            ...
            {step === 1 && (
                <Card onBack={handleBack}>
                    ...
                    {/* ✅ College Selection - Same as Login Page */}
                    <label className="block mb-2 text-blue-900 text-sm font-semibold">College</label>
                    <Select value={college} onValueChange={setCollege}>
                        <SelectTrigger className="w-full bg-white text-blue-900 border-blue-400 h-10 px-3 text-sm rounded-lg focus:ring-2 focus:ring-cyan-200 mb-4">
